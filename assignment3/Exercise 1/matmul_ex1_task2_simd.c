...
/* Task 2: OpenMP SIMD version */
static void matmul_omp_simd(const double *A, const double *B, double *C, int n) {
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            double sum = 0.0;
...
int main(int argc, char **argv) {
    ...
    double *C_simd = (double *)malloc(bytes);
    ...
    matmul_omp_simd(A, B, C_simd, n);
    double t3 = omp_get_wtime();

    int ok = verify(C_serial, C_simd, n);
